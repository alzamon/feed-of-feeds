...
def test_no_id_duplication_in_serialization():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert "id" not in feed_config, \
            "feed.json should not contain 'id' field"
        ...
        assert "id" not in union_config, \
            "union.json should not contain 'id' field"
        ...
        assert "id" not in filter_config, \
            "filter.json should not contain 'id' field"


def test_directory_names_used_for_feed_ids():
    """Test that feed IDs are derived from directory names during load."""
    with tempfile.TemporaryDirectory() as temp_dir:
        config_manager = ConfigManager(temp_dir)
        article_manager = ArticleManager(config_manager)
        ...
        assert loaded_feed.id == "my_test_feed", \
            f"Expected 'my_test_feed', got '{loaded_feed.id}'"
        assert loaded_feed.title == "Test Feed"


def test_root_feed_special_case():
    """Test that root feeds get 'root' as their ID regardless of directory."""
    with tempfile.TemporaryDirectory() as temp_dir:
        config_manager = ConfigManager(temp_dir)
        article_manager = ArticleManager(config_manager)
        loader = FeedLoader(article_manager)
        ...
        loaded_feed = loader.load_feed_from_directory(
            tree_dir, feedpath=[], is_root=True)

        # Verify the root feed gets 'root' as ID, not 'tree'
        assert loaded_feed is not None
        assert loaded_feed.id == "root", \
            f"Root feed should have ID 'root', got '{loaded_feed.id}'"
