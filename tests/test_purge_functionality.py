...
def test_syndication_feed_purge_with_explicit_purge_age():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with sqlite3.connect(article_manager.db_file) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM cache WHERE feedpath = ?", (json.dumps(["test_feed"]),))
        ...
        with sqlite3.connect(article_manager.db_file) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT id FROM cache WHERE feedpath = ?", (json.dumps(["test_feed"]),))
            remaining_ids = [row[0] for row in cursor.fetchall()]
            assert remaining_ids == ["new_article"], "Only new article should remain"
...
def test_syndication_feed_purge_with_default_purge_age():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Create a syndication feed without explicit purge_age (should default to 2 * max_age)
        feed = SyndicationFeed(
            id="test_feed",
            title="Test Feed", 
            description="A test feed",
            last_updated=datetime.now(),
            url="http://example.com/feed.xml",
            max_age=timedelta(days=7),  # max_age = 7 days, so default purge_age = 14 days
            article_manager=article_manager,
            feedpath=["test_feed"]
        )
        ...
        very_old_article = Article(
            ...
            author="Author",
            published_date=now - timedelta(days=20),  # Older than default purge_age (14 days)
            feed_id="test_feed",
            feedpath=["test_feed"]
        )
        ...
        old_article = Article(
            ...
            link="http://example.com/old",
            author="Author",
            published_date=now - timedelta(days=10),  # Between max_age and purge_age
            feed_id="test_feed",
            feedpath=["test_feed"]
        )
        ...
        # Cache the articles
        article_manager.cache_articles([very_old_article, old_article, new_article])
        ...
        with sqlite3.connect(article_manager.db_file) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT id FROM cache WHERE feedpath = ? ORDER BY id", (json.dumps(["test_feed"]),))
            remaining_ids = [row[0] for row in cursor.fetchall()]
            assert set(remaining_ids) == {"new_article", "old_article"}, "New and old articles should remain"
...
def test_feed_manager_purge_old_articles():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Cache the article using the correct feedpath (should be empty list for root feed)
...
def test_no_purge_when_no_max_age_or_purge_age():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Try to purge - should not purge anything since no max_age or purge_age
        purged_count = article_manager.purge_old_articles(feed)
