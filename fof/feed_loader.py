...
class FeedLoader:
    ...
    def load_feed_from_directory(
        ...
        if os.path.isfile(union_path):
            return self._load_union_feed(
                path, feedpath, parent_max_age, is_root)
        elif os.path.isfile(feed_path):
            return self._load_syndication_feed(
                path, feedpath, parent_max_age, is_root)
        elif os.path.isfile(filter_path):
            return self._load_filter_feed(
                path, feedpath, parent_max_age, is_root)
        else:
            logger.error(f"Unknown feed directory structure at {path}")
            return None

    def _load_union_feed(
            self,
            path: str,
            feedpath: list,
            parent_max_age=None,
        ...
        for sub_name, weight in weights.items():
            ...
            sub_feed = self.load_feed_from_directory(
    ...
    def _load_filter_feed(
        ...
        source_feed = self.load_feed_from_directory(
