...
class UnionFeed(BaseFeed):
    ...
    def from_config_dict(
        ...
        member_feeds = []
        for wf_config in config.get("feeds", []):
            weight = wf_config.get("weight", 10.0)
            feed_config = wf_config["feed"]
            ft = FeedType(feed_config["feed_type"])
            if ft == FeedType.REGULAR:
                feed = RegularFeed.from_config_dict(feed_config, article_manager, feed_max_age, feedpath)
            elif ft == FeedType.UNION:
                feed = UnionFeed.from_config_dict(feed_config, article_manager, feed_max_age, feedpath)
            elif ft == FeedType.FILTER:
                feed = FilterFeed.from_config_dict(feed_config, article_manager, feed_max_age, feedpath)
            else:
                logger.warning(f"Unknown feed type {ft} for id {feed_config.get('id')}")
                continue
            member_feeds.append(WeightedFeed(feed, weight))
        return cls(
            id=config["id"],
